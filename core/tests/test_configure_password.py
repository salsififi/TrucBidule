...
from core.password_configurator.password_configurator import configure_password
...
def test_nb_digits_greater_than_length():
    with pytest.raises(ConfigurePasswordArgumentsError) as exc_info:
        configure_password(length=LENGTH, nb_digits=LENGTH + 1, nb_uppers=NB_UPPERS, nb_special_chars=NB_SPECIAL_CHARS)
...
def test_nb_uppers_greater_than_length():
    with pytest.raises(ConfigurePasswordArgumentsError) as exc_info:
        configure_password(length=LENGTH, nb_digits=NB_DIGITS, nb_uppers=LENGTH + 1, nb_special_chars=NB_SPECIAL_CHARS)
...
def test_nb_special_chars_greater_than_length():
    with pytest.raises(ConfigurePasswordArgumentsError) as exc_info:
        configure_password(length=LENGTH, nb_digits=NB_DIGITS, nb_uppers=NB_UPPERS, nb_special_chars=LENGTH + 1)
